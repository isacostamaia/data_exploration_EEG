...
class Filter:
    ...
    def fit(self, class_ = "Target"):
        ...
        if not self.spatial:
            ...
            epochs_data = epochs_data.reshape(e,t,c)
            X_bar_class = X_bar_class.reshape(t,c)
        
        covs = np.stack([e_i@e_i.T/e_i.shape[-1] for e_i in epochs_data]) #n_epochs x n_channels x n_channels if spatial
    ...
    def apply(self,epochs):
        ...
        if self.spatial:
            ...
            epoch_denoiser = lambda epoch: epoch@(self.A_p)@(self.B_p)
